...
export class Swapper implements ISwapper {
  ...
  private async init(): Promise<SwapOutput> {
    switch (this.action) {
      ...
      case Actions.CounterpartyCanInitiate: {
        ...
        if (!toWallet) {
          throw new Error(`No ${this.order.followerAtomicSwap.chain} wallet`);
        }
      ...
      default: {
        throw new Error(SwapperErrors.INVALID_ACTION('init', this.status));
      }
    }
  }
  ...
  private async redeem(): Promise<SwapOutput> {
    ...
    const followerWallet = this.getWallet(this.order.followerAtomicSwap.chain);
    ...
    switch (this.action) {
      case Actions.UserCanRedeem: {
        ...
        return {
          ...
          output: await swap.redeem(secret, this.order.userBtcWalletAddress),
        };
      }
      case Actions.CounterpartyCanRedeem: {
        if (!this.order.secret) throw new Error('Secret not found in order');
        const swap = await htlcWalletFromOrder(
          initiatorWallet,
          this.order,
          User.NATIVE,
      ...
      default: {
        throw new Error(SwapperErrors.INVALID_ACTION('redeem', this.status));
      }
    }
  }
...
const htlcWalletFromOrder = async (
  ...
  const atomicSwap =
    user === User.NATIVE ? order.initiatorAtomicSwap : order.followerAtomicSwap;
