...
export class GardenHTLC implements IHTLCWallet {
    ...
    static async from(
        ...
        secretHash = secretHash.startsWith('0x')
            ? secretHash.slice(2)
            : secretHash;
    ...
    private async _buildRawTx(receiver: string, fee?: number) {
        ...
        if (balance === 0)
            throw new Error(`${address} ${htlcErrors.notFunded}`);
    ...
    private async buildRawTx(fee?: number) {
        return await this._buildRawTx(await this.signer.getAddress(), fee);
    }
    ...
    async instantRefund(
        ...
        for (let i = 0; i < tx.ins.length; i++) {
            ...
            if (
                ...
                throw new Error(
                    htlcErrors.invalidCounterpartySigForUTXO(
                        counterPartySigs[i].utxo
                    )
                );
            }
            ...
            const counterPartySig = counterPartySigs.find(
                (sig) => sig.utxo === txid
            );
            if (!counterPartySig)
