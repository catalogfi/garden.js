...
export class SwapConfig {
    ...
    constructor(
        ...
        try {
            ...
            throw new Error("Error decoding redeemer. Ensure it is a valid base58 encoded", { cause });
        }
        try {
...
export function validateSecret(secret: string): number[] {
    ...
    try {
        ...
        throw new Error("Error decoding secret. Ensure it is a hex string", { cause });
    }
    if (secretBuf.length != 32) {
        throw new Error("Invalid Secret size. Expected 32 bytes, got " + secretBuf.length);
    }
    return secretBuf;
}