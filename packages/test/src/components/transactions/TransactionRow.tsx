...
const explorerUrl = {
  mainnet: 'https://explorer.garden.finance/',
  testnet: 'https://testnet-explorer.garden.finance/',
  localnet: 'http://api.localnet.explorer/',
}
...
export const TransactionRow: FC<TransactionProps> = ({
  ...
  const handleTransactionClick = () => {
    if (order?.create_order?.create_id) {
      window.open(
        `${explorerUrl[environment as keyof typeof explorerUrl]}order/${create_order.create_id}`,
        '_blank',
      );
    }
  };
