...
const Transaction: React.FC = () => {
  ...
  }, [orderBook, garden, fetchAndSetOrders, fetchAndSetBlockNumbers]);

  return (
    <div className="flex flex-col justify-center gap-5 overflow-hidden h-full min-h-[inherit] max-h-[50vh] p-8 bg-white/50 rounded-2xl">
      <h1 className="text-xl font-bold">Transaction History</h1>
      <div className="flex flex-col overflow-y-auto h-full max-h-[inherit] bg-white/60 rounded-2xl scrollbar-thin scrollbar-thumb-[#E36492] scrollbar-track-gray-200">
        {isLoadingOrders ? (
          ...
          orders.map((order, index) => (
            <div key={index} className="w-full">
              ...
              {index !== orders.length - 1 ? (
                <div className="bg-[#E36492] w-full h-px"></div>
              ) : null}
            </div>
          ))
        )}
